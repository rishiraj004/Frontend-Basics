...
todoInputBar.addEventListener("keydown",function getTextAndAddTodo(event) {
    if(event.key=='Enter'){
        let todotext = todoInputBar.value;
        if(todotext.length == 0) return;
        let todo = {text: todotext, status: 'In progress', finishButtontext: 'Finshed', editButtonText: 'Edit'};
        todos.push(todo);
...
function editTodo(event) {
    ...
    if(todos[indexToEdit].editButtonText == "Edit"){
        todos[indexToEdit].editButtonText = "Save";
        reRenderTodos();
    ...
    else if(todos[indexToEdit].editButtonText == "Save"){
        todos[indexToEdit].editButtonText = "Edit";
        let detailDiv = document.querySelector(`div[todo-idx="${indexToEdit}"]`);
        let input = document.querySelector(`input[todo-idx="${indexToEdit}"]`);detailDiv.textContent = input.value;
...
function saveEdittedTodo(event) {
    ...
    if(event.keyCode == 13) {
        if(todos[indexToEdit].editButtonText == "Save"){
            todos[indexToEdit].editButtonText = "Edit";
            let detailDiv = document.querySelector(`div[todo-idx="${indexToEdit}"]`);
            let input = document.querySelector(`input[todo-idx="${indexToEdit}"]`);detailDiv.textContent = input.value;
            todos[indexToEdit].text = input.value;
...
function addTodo(todo, todoCount) {
    ...
    editButton.textContent = todo.editButtonText;
